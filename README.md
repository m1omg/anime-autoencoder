...
- **Architecture:** `6912 → 128 → 32 → 2 → 32 → 128 → 6912` (48×48 RGB in and out),
  ...
  on flat `Float32Array`s, zero dependencies. 48×48 with a full batch of 8 is the sweet spot:
  it converges fast enough to *watch*.
- **Two compute backends (toggleable live):**
  - **🧵 Web Worker** (default) — the trainer runs in a background thread via a
    MessageChannel-scheduled loop, so the UI stays smooth no matter how hard it's training.
  - **🧮 CPU** — same trainer on the main thread (`?mode=cpu`), the simplest possible setup.
  Whichever backend trains, it periodically hands a weight snapshot to the main thread,
